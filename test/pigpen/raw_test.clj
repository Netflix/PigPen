...
(deftest test-tree->command
  (with-redefs [pigpen.raw/pigsym (pigsym-inc)]
    ...
    (test-diff
      (->
        (load$ "foo" '[foo] default-storage {})
        (store$ "bar" default-storage {})
        (tree->command))
      '{:type :store
        :id store2
        :description "bar"
...
(deftest test-command->required-fields
  ...
  (is (= nil
         (command->required-fields
           (load$ "foo" '[a0 b0 c0 d0] default-storage {})))))
...
(deftest test-command->references
  ...
  (test-diff
    (command->references (generate$ {} [(projection-func$ 'foo
                                          (code$ String ['foo]
  ...
  (test-diff
    (command->references (store$ {} "" (storage$ ["my-jar.jar"] "f" []) {}))
    ["my-jar.jar"]))
